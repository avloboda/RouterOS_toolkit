...
def runcode(command):    # this is where the script will send API calls to the device.
	try:
...
for device in devices_list:
	...
	try:
		...
		with open('complete_list-{}.txt'.format(current_date), 'a') as file: # create a file and write the output
	...
	if architecture == 'arm':
		try:
			...
			with open('arm_devices_list-{}.txt'.format(current_date), 'a') as file: # create a file and write the output
				file.write('{}\t{}\t{}\t{}\t{}\n'.format(device,hostname,board_name,architecture,version))
		except Exception as unknown_error:
			print(unknown_error)
	...
	if architecture == 'mipsbe':
		try:
			...
			with open('mipsbe_devices_list-{}.txt'.format(current_date), 'a') as file: # create a file and write the output
				file.write('{}\t{}\t{}\t{}\t{}\n'.format(device,hostname,board_name,architecture,version))
		except Exception as unknown_error:
			print(unknown_error)
	...
	if architecture == 'tile':
		try:
			...
			with open('tile_devices_list-{}.txt'.format(current_date), 'a') as file: # create a file and write the output
				file.write('{}\t{}\t{}\t{}\t{}\n'.format(device,hostname,board_name,architecture,version))
		except Exception as unknown_error:
			print(unknown_error)	
